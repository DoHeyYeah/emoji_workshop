...
class TagPanel(QWidget):
    ...
    def _normalize_tag_name(self, text: str) -> str:
        """清洗标签显示文本，兼容半角/全角括号计数：开心 (3) / 开心（3/10） -> 开心"""
        s = (text or "").strip()
        s = re.sub(r"\s*\((?:\d+|\d+\s*/\s*\d+)\)\s*$", "", s)   # 半角
        s = re.sub(r"\s*（(?:\d+|\d+\s*/\s*\d+)）\s*$", "", s)   # 全角
        return s.strip()
    ...
    def setup_ui(self):
        ...
        self.tag_mode_radio = QRadioButton("✏️打标签模式")
        ...
        self.mode_hint_label = QLabel("已选中0个标签作筛选条件")
        ...
        self.assign_btn = QPushButton("🏷️添加到选中图片")
    ...
    def add_tag(self):
        name = self._normalize_tag_name(self.tag_input.text())
        if not name:
    ...
    def load_tags(self):
        self.tag_list.clear()
        for tag_id, name, _ in self.db.get_all_tags():
            clean_name = self._normalize_tag_name(name)
    ...
    def _selected_tag_names_clean(self):
        names = [self._normalize_tag_name(it.text()) for it in self._selected_tag_items()]
    ...
    def on_selection_changed(self):
        selected_items = self._selected_tag_items()
        selected_ids = [it.data(Qt.ItemDataRole.UserRole) for it in selected_items]
        selected_names = self._selected_tag_names_clean()
        ...
        if self.mode == "filter":
            ...
            self.assign_btn.setEnabled(bool(self.current_image_ids) and bool(selected_ids))
    ...
    def set_current_images(self, image_ids: list):
        ...
        if n == 0:
            ...
            self.selection_label.setText("已选中1张图片")
        else:
    ...
    def load_image_tags_multi(self, image_ids: list):
        ...
        # 单图
        if len(image_ids) == 1:
            for _, name, _ in self.db.get_image_tags(image_ids[0]):
                self.image_tag_list.addItem(QListWidgetItem(self._normalize_tag_name(name)))
            return

        # 多图
        tag_counts: dict[int, dict] = {}
        total = len(image_ids)
        for img_id in image_ids:
            rows = self.db.get_image_tags(img_id)
            one_image_tags = {row[0]: row for row in rows}
            for tag_id, row in one_image_tags.items():
                if tag_id not in tag_counts:
                    tag_counts[tag_id] = {"row": row, "count": 0}
                tag_counts[tag_id]["count"] += 1

        for info in tag_counts.values():
            _, name, _ = info["row"]
            name = self._normalize_tag_name(name)
            cnt = info["count"]
            label = name if cnt == total else f"{name}（{cnt}/{total}）"
            self.image_tag_list.addItem(QListWidgetItem(label))
    ...
    def assign_tags_to_image(self):
        ...
        if not ids:
            ...
            return
        selected_names = self._selected_tag_names_clean()
        typed_name = self._normalize_tag_name(self.tag_input.text())
        if typed_name and typed_name not in selected_names:
            selected_names.append(typed_name)
        if not selected_names:
            QMessageBox.warning(self, "提示", "请先选择标签")
            return
        # 关键：保存打标签前的筛选状态（仅筛选模式）
        keep_filter_names = []
        keep_filter_mode = self.match_mode
        if self.mode == "filter":
            keep_filter_names = self._selected_tag_names_clean()
        ...
        # 关键：恢复筛选选择（避免 load_tags 清空选择导致主界面回到全量）
        if self.mode == "filter" and keep_filter_names:
            self.tag_list.blockSignals(True)
            for i in range(self.tag_list.count()):
                item = self.tag_list.item(i)
                item.setSelected(self._normalize_tag_name(item.text()) in keep_filter_names)
            self.tag_list.blockSignals(False)
        # 发一次统一更新
        self.tags_updated.emit()
        # 若在筛选模式，主动再触发一次筛选信号，确保画廊立刻按标签刷新
        if self.mode == "filter":
            self.on_selection_changed()
    ...
    def show_context_menu(self, position):
        ...
        menu = QMenu(self)
        delete_action = menu.addAction("🗑️删除标签")
