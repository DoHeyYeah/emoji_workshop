...
class GalleryView(QWidget):
    ...
    def filter_by_tag_names(self, tag_names: List[str], match_mode: str = "union"):
        """按标签筛选，支持并集/交集（自动清洗如 '标签 (3)' 的显示文本）"""
        cleaned = []
        for t in (tag_names or []):
            n = self._normalize_tag_name(t)
            if n:
                cleaned.append(n)
        ...
        if match_mode == "intersect":
            rows = self.db.get_images_by_tags_intersect(cleaned)
        else:
            rows = self.db.get_images_by_tags_union(cleaned)
