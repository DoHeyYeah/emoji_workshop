...
class RecommendController:
    ...
    def recommend(self, context: str, top_k: int = 3) -> list[ImageModel]:
        ...
        if llm_cfg.get("enabled"):
            if llm_cfg.get("api_key"):
                ...
                try:
                    ...
                    llm_status = "失败"
                    fallback_notes.append("LLM调用失败，已降级本地召回")
                    logging.debug("[RecommendController] LLM分析失败，降级本地召回: %s", exc)
            else:
                llm_status = "未配置Key"
                fallback_notes.append("LLM已启用但未配置 API Key")
        ...
        if llm_status == "成功" and llm_seed_count == 0:
            fallback_notes.append("LLM粗筛无有效候选，已降级本地文本匹配")
        elif llm_status == "成功" and llm_seed_count < candidate_count:
            fallback_notes.append("LLM候选不足，已使用本地文本匹配补齐")
        vision_status = "未启用"
