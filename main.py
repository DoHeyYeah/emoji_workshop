...
class MainWindow(QMainWindow):
    ...
    def on_tags_updated(self):
        """
        标签更新后刷新当前画廊：
        - 若当前是筛选模式且已有选中标签：保持筛选结果
        - 否则：全量刷新
        """
        selected_items = self.tag_panel.tag_list.selectedItems()
        selected_names = [self.tag_panel._normalize_tag_name(it.text()) for it in selected_items]
        selected_names = [n for n in selected_names if n]

        if self.tag_panel.mode == "filter" and selected_names:
            self.gallery.filter_by_tag_names(selected_names, self.tag_panel.match_mode)
        else:
            self.gallery.load_from_database()
