...
class LLMService:
    ...
    EMPTY_ANALYSIS = {"tags": [], "keywords": [], "image_ids": [], "reason": ""}
    ...
    def recommend_tags(self, context: str, available_tags: list[str], top_k: int = 5) -> list[str]:
...
- 如果没有完全匹配的，挑选语义最接近的"""
    ...
    def analyze_recommendation(
        ...
        summary_lines, _ = self._build_summary_lines_and_valid_ids(summaries)
    ...
    def select_candidate_images(
        ...
        summary_lines, valid_ids = self._build_summary_lines_and_valid_ids(image_summaries)
        if not summary_lines:
            return dict(self.EMPTY_ANALYSIS)
        system = (
            "你是表情包候选筛选助手。"
        ...
        prompt = (
            ...
            "4) 初筛范围是上面列出的整个图片库，不要只关注前几项；\n"
            "5) 主要根据表情包名称判断是否相关，标签作为辅助；\n"
            "6) 只做候选筛选，不做最终排序；\n"
            "7) 用户输入的关键词/描述/上下文是最高优先级。"
        )
        response = self.chat(
            prompt,
            system=system,
        ...
        parsed["image_ids"] = self._filter_valid_image_ids(parsed["image_ids"], valid_ids, candidate_count)
        parsed["tags"] = parsed["tags"][: self.MAX_CANDIDATE_TAGS]
        parsed["keywords"] = parsed["keywords"][: self.MAX_CANDIDATE_KEYWORDS]
        return parsed
    @staticmethod
    def _build_summary_lines_and_valid_ids(image_summaries: list[dict]) -> tuple[list[str], set[int]]:
        ...
        return summary_lines, valid_ids
    @staticmethod
    def _filter_valid_image_ids(
        image_ids: list[int], valid_ids: set[int], limit: int
    ) -> list[int]:
    ...
    def _parse_analysis_response(text: str, available_tags: list[str], top_k: int) -> dict:
        ...
        if not payload:
            return dict(LLMService.EMPTY_ANALYSIS)
        try:
