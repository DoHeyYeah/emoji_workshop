...
class DatabaseService:
    ...
    def search_images_by_tags(self, tag_ids: list[int]) -> list[tuple]:
        ...
        """
        return self._fetch_images(query, [*tag_ids, len(set(tag_ids))])
    def get_images_by_tags_union(self, tag_names: list[str]) -> list[dict]:
        ...
        query = f"""
            SELECT DISTINCT i.{self.IMAGE_COLUMNS}
            FROM images i
            JOIN image_tags it ON i.id = it.image_id
            ...
            ORDER BY i.created_at DESC
        """
        return self._fetch_image_dicts(query, tag_names)
    def get_images_by_tags_intersect(self, tag_names: list[str]) -> list[dict]:
        ...
        query = f"""
            ...
            ORDER BY i.created_at DESC
        """
        return self._fetch_image_dicts(query, [*tag_names, len(set(tag_names))])
    ...
    def search_by_keywords(self, keywords: list[str], top_k: int = 3) -> list[tuple]:
        ...
        query = f"""
            SELECT i.{self.IMAGE_COLUMNS}, COUNT(DISTINCT t.id) AS match_count
            FROM images i
            JOIN image_tags it ON i.id = it.image_id
