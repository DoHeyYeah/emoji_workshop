...
class ReplicateService:
    ...
    def __init__(self, base_url: str, api_key: str, model: str):
        ...
        self.model = (model or "").strip() or "fofr/sticker-maker"
    ...
    def _pick_output_url(output) -> str:
        ...
        if not isinstance(output, list) or not output:
            return ""
        gif_url = next(
            (item for item in output if isinstance(item, str) and ".gif" in item.lower()),
            None,
        )
        if gif_url:
            return gif_url
        first_url = next((item for item in output if isinstance(item, str)), "")
        return first_url
