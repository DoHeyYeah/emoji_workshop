...
class VisionService:
    ...
    def rerank(
        ...
        limit = min(max_images, self.MAX_IMAGES) if max_images and max_images > 0 else self.MAX_IMAGES
        ...
        ranked_ids = self._parse_ranking(response_text, list(indexed.keys()))[:top_k]
                ...
                "content": [
                    {
                        ...
                        "text": (
                            f"用户输入/聊天上下文/关键词描述:\n{context}\n\n"
                            f"上面共有 {image_count} 张候选表情包(编号 1 到 {image_count})。\n\n"
