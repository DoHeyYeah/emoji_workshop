...
class PersonalityService:
    ...
    def _build_dimensions(self, tag_counter: Counter, peak_period: str) -> dict[str, int]:
        ...
        return {dim: max(10, min(95, int(round(score)))) for dim, score in base.items()}
    @staticmethod
    def _build_evidence_tags(tag_counter: Counter) -> list[tuple[str, float]]:
        total = sum(tag_counter.values()) or 1
        return [(str(tag), round(count / total, 3)) for tag, count in tag_counter.most_common(8)]
    def _build_fallback_traits(self, dimensions: dict[str, int]) -> list[str]:
        traits = [
            self.TRAIT_BY_DIMENSION.get(dim, dim)
            for dim, score in sorted(dimensions.items(), key=lambda x: x[1], reverse=True)[:4]
            if score >= 50
        ]
